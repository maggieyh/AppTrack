...
// will be called once the user has tapped the login button and the server has responded with either a success or failure message. We'll then either receive an error or a user, which will allow us to determine if the login was successful.
...
class ParseLoginHelper : NSObject {
    ...
    var fbLogInUserDic: NSDictionary!
...
//                
                if let fbUsername = result?["name"] as? String {
                    ...
                    self.fbLogInUserDic = result as! NSDictionary
                    print(self.fbLogInUserDic)
